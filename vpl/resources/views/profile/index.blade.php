...
<div class="w-full my-8 p-6 rounded-md shadow bg-white">
  ...
  <div class="mt-4 flex flex-col">
    <form action="{{ route('update_profile', ['id' => $user->id]) }}" method="POST" enctype="multipart/form-data" id="profile-form">
      @csrf
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <div class="flex items-center">
          @if($user->avatar)
            <img src="{{ asset('storage/avatars/' . $user->avatar) }}" alt="User Avatar" class="h-16 w-16 rounded-full object-cover">
          @else
