...
$(document).ready(function() {
  ...
  $('button[onclick="showAddPaymentMethodForm()"]').click(function() {
    $('#payment-form').hide(); // Hide existing payment form
    $('#add-payment-form').show(); // Show new payment form
  });
});
  ...
  <form action="" method="GET">
    <div class="mt-4 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-12">
      ...
      <div class="flex justify-between sm:col-span-12">
        <button id="viewCardButton" type="button" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-0">View Card</button>
...
<div id="cardModal" class="fixed overflow-y-auto overflow-x-hidden sm:overflow-hidden z-10 inset-0 flex justify-center sm:items-center hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
  ...
  <div id="confirm-modal" class="absolute my-8 bg-white transition ease-in rounded-md p-4 z-10 w-11/12 max-w-lg sm:max-w-2xl lg:max-w-3xl scale-0">
    ...
    <div class="w-full mt-2 px-2">
      ...
      <div id="add-payment-form" class="hidden mt-4">
        <h4 class="text-lg font-medium text-gray-700">Add New Card</h4>
        <form id="new-payment-form" class="mt-2 border rounded-md p-4">
          @csrf
          <div class="mb-4">
            <label for="card-number" class="block text-sm font-medium text-gray-700">Card Number</label>
            <input type="text" id="card-number" name="card_number" class="shadow-sm focus:ring-cyan-600 focus:border-cyan-600 block w-full sm:text-sm border-gray-300 rounded-md" required>
          </div>
          <div class="mb-4">
            <label for="expiry-date" class="block text-sm font-medium text-gray-700">Expiry Date</label>
            <input type="text" id="expiry-date" name="expiry_date" class="shadow-sm focus:ring-cyan-600 focus:border-cyan-600 block w-full sm:text-sm border-gray-300 rounded-md" placeholder="MM/YY" required>
          </div>
          <div class="mb-4">
            <label for="cvv" class="block text-sm font-medium text-gray-700">CVV</label>
            <input type="text" id="cvv" name="cvv" class="shadow-sm focus:ring-cyan-600 focus:border-cyan-600 block w-full sm:text-sm border-gray-300 rounded-md" required>
          </div>
          <button type="submit" class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-cyan-600 hover:bg-cyan-700 focus:outline-none focus:ring-0">Add</button>
        </form>
      </div>
    </div>
  </div>
