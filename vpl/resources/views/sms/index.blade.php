...
<div class="w-full my-8 p-6 rounded-md shadow bg-white">
  <div class="flex justify-between items-center">
    <h3 class="px-1 text-lg leading-6 font-medium text-gray-900">Search SMS</h3>
  </div>
  <form action="" method="GET">
    ...
    <div class="mt-4 flex flex-wrap gap-4">
      <div style="width: 250px;">
        ...
        <div class="mt-1">
          <select id="number" name="search_number" class="shadow-sm focus:ring-cyan-600 focus:border-cyan-600 block w-full sm:text-sm border-gray-300 rounded-md">
      ...
      <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200 text-gray-800 font-medium">
              @foreach($messages as $message)
              <tr>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $message->number->number }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $message->from_number }}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $message->content}}</td>
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $message->received_at->format('Y-m-d H:i:s') }}</td>
